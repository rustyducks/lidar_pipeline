...
pub fn mask_from_file(filename: &str) -> Mask {
    ...
    let table = BoundingBox::new(
        doc["mask"]["table"]["x_start"]
            .as_f64()
            .expect("x_start parameter of the table unreadable"),
        doc["mask"]["table"]["y_start"]
            .as_f64()
            .expect("y_start parameter of the table unreadable"),
        doc["mask"]["table"]["x_stop"]
            .as_f64()
            .expect("x_stop parameter of the table unreadable"),
        doc["mask"]["table"]["y_stop"]
            .as_f64()
            .expect("y_stop parameter of the table unreadable"),
    );
    let mut static_obstacles: Vec<Box<dyn Obstacle>> = Vec::new();
    let obss = doc["mask"]["static_obstacles"].as_vec();
    match obss{
        Some (oobss) => {for obs in oobss{
